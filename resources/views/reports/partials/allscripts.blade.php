...
jQuery(document).ready(function($){
    ...
    var url = "{!! route('reports.data', ['all-reports']) !!}";
    ...
    var oTable = $(table_selector).dataTable({
        ...
        serverSide: true,
        "ajax": {
            "url": "{!! route('reports.data', ['all-reports']) !!}",
            "data": function ( d ) {
                return $.extend( {}, d, {
                    "status": $('#status').val()
                } );
    ...
    $('#btn-apply').on('click', function (e) {
        ...
        if (rows_selected.length == 0) {
            ...
            swal({
                ...
                title: "Are you sure?",
                text: rows_selected.length + " reports will be deleted permanently from this system!",
                type: "warning",
                ...
                $.ajax({
                    url: "{{route('reports.destroy-all') }}",
                    data: {
                        reports: rows_selected // will be accessible in $_POST['data1']
                    },
                    headers: {'X-XSRF-TOKEN': '{{\Illuminate\Support\Facades\Crypt::encrypt(csrf_token())}}'},
                    error: function () {
                        swal("Cancelled", "{{trans('reports.index.delete_unable')}}", "error");
                    },
                    success: function (response) {
                        if (response.success == 'true') {
                            rows_selected = [];
                            oTable.fnDraw();
                            swal("{{trans('reports.index.delete_validation')}}", "{{trans('reports.index.delete_msg')}}", "success");
                        } else {
                            swal("Cancelled", "{{trans('reports.index.delete_unable')}}", "error");
                        }
                    },
    ...
    $(".tbl-reports tbody").on('click', 'input[type="checkbox"]', function (e) {
    ...
    $('.table').on('click', '.report-delete', function(e){
        e.preventDefault();
        ...
        swal({
            showLoaderOnConfirm: true,
            title: "Are you sure?",
            text: "This report will be deleted permanently from this system!",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#007AFF",
            confirmButtonText: "Yes, delete it!",
            ...
            $.ajax({
                url:  '{{ str_replace('-1','',route('reports.destroy',-1))  }}'+reportId,
                headers: { 'X-XSRF-TOKEN' : '{{\Illuminate\Support\Facades\Crypt::encrypt(csrf_token())}}' },
                error: function() {
                    swal("Cancelled", "{{trans('reports.index.delete_unable')}}", "error");
                },
