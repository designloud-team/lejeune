...
jQuery(document).ready(function($){
    ...
    $('#btn-apply').on('click', function (e) {
        ...
        if (rows_selected.length == 0) {
            ...
            swal({
                ...
                title: "Are you sure?",
                text: rows_selected.length + " jobs will be deleted permanently from this system!",
                type: "warning",
                ...
                $.ajax({
                    url: "{{route('jobs.destroy-all') }}",
                    data: {
                        jobs: rows_selected // will be accessible in $_POST['data1']
                    },
                    headers: {'X-XSRF-TOKEN': '{{\Illuminate\Support\Facades\Crypt::encrypt(csrf_token())}}'},
                    error: function () {
                        swal("Cancelled", "{{trans('jobs.index.delete_unable')}}", "error");
                    },
                    success: function (response) {
                        if (response.success == 'true') {
                            rows_selected = [];
                            oTable.fnDraw();
                            swal("{{trans('jobs.index.delete_validation')}}", "{{trans('jobs.index.delete_msg')}}", "success");
                        } else {
                            swal("Cancelled", "{{trans('jobs.index.delete_unable')}}", "error");
                        }
                    },
    ...
    $(".tbl-jobs tbody").on('click', 'input[type="checkbox"]', function (e) {
    ...
    $('.table').on('click', '.job-delete', function(e){
        e.preventDefault();
        ...
        swal({
            showLoaderOnConfirm: true,
            title: "Are you sure?",
            text: "This job will be deleted permanently from this system!",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#007AFF",
            confirmButtonText: "Yes, delete it!",
            ...
            $.ajax({
                url:  '{{ str_replace('-1','',route('jobs.destroy',-1))  }}'+jobId,
                headers: { 'X-XSRF-TOKEN' : '{{\Illuminate\Support\Facades\Crypt::encrypt(csrf_token())}}' },
                error: function() {
                    swal("Cancelled", "{{trans('jobs.index.delete_unable')}}", "error");
                },
